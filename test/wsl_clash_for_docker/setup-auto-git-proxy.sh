...
# Description:
# 1. 脚本功能说明：
#   - 本脚本用于在 WSL 中设置 Git 的代理，以便在 WSL 中使用 Clash 代理 Git 的网络请求。
#   - WSL 系统每次启动时，都会运行更新脚本，找寻本次启动被分配的网卡 IP 地址，设置 Git 的代理服务器地址。
# 2. 使用方法：
#   - 请将本脚本放置在 WSL 中的任意目录下，并执行 `sudo bash setup-auto-docker-proxy.sh`。
# 3. 卸载方法：
#   - 请执行 `sudo bash setup-auto-git-proxy.sh uninstall`。

### 1 初始化变量
# 定义更新脚本和配置文件路径
SCRIPT_PATH="/usr/local/bin/update-git-proxy.sh" # 脚本路径
SERVICE_PATH="/etc/systemd/system/update-git-proxy.service"  # 服务文件路径
...
# 输出提示信息
echo "auto-git-proxy Setup completed."
