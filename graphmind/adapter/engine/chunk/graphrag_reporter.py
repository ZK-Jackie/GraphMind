...
from graphmind.adapter.engine.reporter import BaseReporter, GraphmindReporter
from graphrag.index.progress import ProgressReporter


class GraphragReporter(ProgressReporter, BaseReporter):
    ...
    def child(self, prefix: str, transient: bool = True) -> ProgressReporter:
        ...
        return GraphragReporter(parent=self, prefix=prefix, transient=transient)
    ...
    def __call__(self, progress_update: DSProgress) -> None:
        ...
        if not hasattr(self, "_now_completed") or self._now_completed != completed:
            # 若当前无进度或进度有变化，则输出进度
            if progress_description != "":
                print(f"{self._prefix} - {completed}/{total} {progress_description}")
            else:
                print(f"{self._prefix} - {completed}/{total}")
            self._now_completed = completed
