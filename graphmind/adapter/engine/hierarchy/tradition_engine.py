...
from graphmind.adapter.engine.support_config import HIERARCHY_STRUCT_SUPPORT
...
class TraditionEngine(BaseEngine):
    ...
    def validate_struct_type(self) -> Self:
        ...
        elif self.struct_type not in HIERARCHY_STRUCT_SUPPORT:
            warnings.warn(
                f"Unsupported structure type: {self.struct_type}, should be one of {HIERARCHY_STRUCT_SUPPORT}")
        return self
    ...
    def failed_retry(self, **kwargs) -> Self:
        failed_task_list = []
        # 1 寻找特定 failed 开头的文件，对应的正则表达式为：failed_.*\.json
        failed_file_list = find_file(self.work_dir, pattern="failed_.*\.json")
        # 读取文件内容
        for failed_file in failed_file_list:
            ...
            try:
                ...
                failed_task_list.append(BaseTask(**data))
            except JSONDecodeError as e:
                warnings.warn(f"Failed to load failed task from file {failed_file}: {e}")

        return self

    def persist_local(self, **kwargs) -> Self:
        """
        本地持久化结果
        """
        # 保存结果
        ...
        for tree in tqdm(info, desc="Moving entity results", unit="doc"):
            for node in tree:
                ...
                if node.entity_task and node.entity_task.task_status == "SUCCESS":
                    ...
                    # 将失败案例写到文件中
                    try:
                        with open(f"{self.work_dir}/failed_entity_{self._execute_failed_cnt}.json", "a",
                                  encoding="utf-8") as f:
        ...
        for tree in tqdm(info, desc="Moving relation results", unit="doc"):
            for node in tree:
                ...
                if node.relation_task and node.relation_task.task_status == "SUCCESS":
                    ...
                    # 将失败案例写到文件中
                    try:
                        with open(f"{self.work_dir}/failed_relation_{self._execute_failed_cnt}.json", "a",
                                  encoding="utf-8") as f:
        ...
        for entity_name in tqdm(entity_dfs, desc="Merging and saving duplicated entities", unit="item"):
            ...
            while j < temp_entity_df.shape[0]:
                ...
                if temp_task.task_status == "SUCCESS":
                    ...
                    # 将失败案例写到文件中
                    try:
                        with open(f"{self.work_dir}/failed_entity_merge_{self._execute_failed_cnt}.json", "a",
                                  encoding="utf-8") as f:
